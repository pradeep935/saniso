...
;(function ($) {
    ...
    MartApp.productQuickView = function () {
        ...
        MartApp.$body.on('click', '.quick-view', function (e) {
            e.preventDefault();
            const _self = $(e.currentTarget);
            _self.addClass('loading');
            $modal.removeClass('loaded').addClass('loading');
    ...
    MartApp.lazyLoad = function (container, init = false) {
        if (init) {
            ...
            // Only pass container if it is a valid element
            let options = {
                elements_selector: '.lazyload',
                callback_error: (img) => {
                    img.setAttribute('src', siteConfig.img_placeholder)
    ...
    MartApp.productsFilter = function () {
        ...
        MartApp.$body.on('click', '.bb-product-filter-link', function (e) {
            e.preventDefault()
            ...
            if (!$form.length) {
                // If no form found, redirect directly
                window.location.href = href
                return
            }
            
            // Update the form and submit
            const $categoryInput = $form.find('input[name="categories"]')
            const $categoryArrayInput = $form.find('input[name="categories[]"]')
            
            // Clear existing category selections
            $form.find('.bb-product-filter-link').removeClass('active')
            ...
            if (categoryId) {
                ...
                } else if ($categoryArrayInput.length) {
                    $categoryArrayInput.val(categoryId)
                } else {
                    // Create hidden input for category
                    $form.append('<input type="hidden" name="categories" value="' + categoryId + '">')
                }
                
                // Trigger form submission
                $form.trigger('submit')
            } else {
                // No category selected, redirect to base URL
                if ($categoryInput.length) {
                    $categoryInput.remove()
                }
                if ($categoryArrayInput.length) {
                    $categoryArrayInput.remove()
                }
                
                // Set the form action to the link href and submit
                $form.attr('action', href).trigger('submit')
            }
        })

        // Handle form submission for filters
        MartApp.$body.on('submit', 'form.bb-product-form-filter', function (e) {
            const $form = $(this)
            ...
            // Let the default submission continue
        })
    }
