...
>
    ...
    @if(Auth::check() && (request()->routeIs('*.admin.*') || request()->is('admin/*') || request()->is('vendor/*')))
        @include('plugins/ecommerce::includes.quote-notifications')
    @endif
    ...
    <script>
        // Register service worker to enable showing an offline page when network is unavailable
        ...
        if ('serviceWorker' in navigator) {
            ...
            if (isSecure) {
                ...
                // Live HTTP (non-HTTPS) servers cannot register service workers.
                // Provide a console hint for administrators.
                console.info('Service worker not registered: service workers require HTTPS or localhost.');
            }
        }
    </script>
</body>
