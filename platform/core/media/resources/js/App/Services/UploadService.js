...
export class UploadService {
    ...
    setupDropZone() {
        ...
        _self.dropZone.on('queuecomplete', () => {
            // Always show newest first after upload
            UploadService.forceSortNewest();
    ...
    // Force sort to newest after upload
    static forceSortNewest() {
        if (window.MediaConfig && window.MediaConfig.request_params) {
            window.MediaConfig.request_params.sort_by = 'created_at-desc';
            if (typeof window.localStorage !== 'undefined') {
                localStorage.setItem('MediaConfig', JSON.stringify(window.MediaConfig));
            }
        }
    }
