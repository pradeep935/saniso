...
class MediaController extends BaseController
{
    ...
    public function getList(MediaListRequest $request)
    {
        ...
        $paramsFile = [
            ...
            'paginate' => [
                'per_page' => min(
                    $request->integer('posts_per_page', config('core.media.pagination.per_page', 50)),
                    config('core.media.pagination.max_per_page', 100)
                ),
        ...
        $response = RvMedia::responseSuccess([
            'files' => $files,
            'folders' => $folders,
        ...
        // Add HTTP caching headers for faster subsequent loads
        return $response->header('Cache-Control', 'private, max-age=300') // 5 minutes
