...
@if ($favicon = theme_option('favicon'))
    @php
        $faviconUrl = RvMedia::getImageUrl($favicon);
        $faviconRealPath = RvMedia::getRealPath($favicon);
        $faviconMime = ($faviconRealPath && File::exists($faviconRealPath))
            ? rescue(fn () => File::mimeType($faviconRealPath), 'image/x-icon')
            : 'image/x-icon';
    @endphp
    {!! Html::favicon($faviconUrl, ['type' => $faviconMime]) !!}
@endif
