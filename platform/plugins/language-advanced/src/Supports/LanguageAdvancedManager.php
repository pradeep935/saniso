...
class LanguageAdvancedManager
{
    ...
    /**
     * Compatibility shim: register translation import/export handlers for a model.
     * Some plugins call this method (e.g. ecommerce HookServiceProvider). Older/newer
     * versions of language-advanced may or may not implement this; provide a no-op
     * implementation to avoid fatal errors during package discovery.
     *
     * @param string $model
     * @param string $label
     * @param array $routes
     * @return bool
     */
    public static function registerTranslationImportExport(string $model, string $label, array $routes = []): bool
    {
        // Best-effort: register the model as supported if not already present.
        $supported = self::getSupported();

        if (! isset($supported[$model])) {
            // default to empty array of columns — real plugins will handle specifics
            config(['plugins.language-advanced.general.supported' => array_merge($supported, [$model => []])]);
        }

        // Return true to signal success for callers expecting this method.
        return true;
    }
}
