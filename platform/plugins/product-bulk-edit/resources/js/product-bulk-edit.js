        ...
        bindEvents() {
            ...
            $('#btn-upload-image').on('click', () => {
                ...
                if (activeTab === 'file-tab') {
                    ...
                    Botble.showNotice('📤 Uploading image...', 'Image upload in progress');
                    ...
                    Botble.showNotice('📥 Importing image from URL...', 'Image import in progress');
            ...
            $('#btn-upload-gallery').on('click', () => {
                ...
                if (activeTab === 'gallery-file-tab') {
                    ...
                    // Close modal immediately
                    const galleryModal = bootstrap.Modal.getInstance($('#galleryModal')[0]);
                    if (galleryModal) galleryModal.hide();

                    // Show upload progress notification
                    Botble.showNotice(`📤 Uploading ${files.length} image(s)...`, 'Gallery upload in progress');
                    ...
                    $.ajax({
                        ...
                        success: (response) => {
                            if (response.error === false) {
                                ...
                                // Update featured image column instantly
                                if (response.data && response.data.featured_image) {
                                    const rowIndex = this.products.findIndex(p => p.id == productId);
                                    if (rowIndex !== -1 && this.hot) {
                                        this.hot.setDataAtRowProp(rowIndex, 'image', response.data.featured_image);
                                    }
                                }
                    ...
                    // Show import progress notification
                    Botble.showNotice(`📥 Importing ${urls.length} image(s) from URLs...`, 'Gallery import in progress');
                        ...
                        if (colDef.key === 'stock_status') {
                            ...
                            cellProperties.renderer = function(instance, td, row, col, prop, value, cellProperties) {
                                ...
                                if (value) {
                                    ...
                                    td.innerHTML = `
                                        <div class="upload-image-trigger" data-product-id="${productId}" style="display: flex; align-items: center; gap: 8px; cursor: pointer; width: 100%;" title="${filename} - Click to change image">
                                            ...
                                            <span style="font-size: 11px; color: #666; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;" title="${filename}">${displayName}</span>
                                        </div>
                                    `;
                                } else {
