...
class ProductExporter extends Exporter
{
    ...
    public function __construct()
    {
        ...
        // Defensive: always reset filters on new instance
    ...
    protected function getChunkedCollection(): Collection
    {
        ...
        $this->getProductQuery()
            ...
            ->with($with)
            ->orderBy('id')
            ->chunk($this->chunkSize, function ($batch) use (&$products) {
    ...
    public function formatProductRow(Product $product): array
    {
        ...
        $result = [
            ...
            'slug' => $product->slug,
            // Defensive: handle missing/broken slugable
            'url' => optional($product->slugable)->url ?? '',
            'sku' => $product->sku,
    ...
    public function productResults(Collection $products): array
    {
        ...
        foreach ($products as $product) {
            // Export every product as a row, regardless of parent/variation
    ...
    {
        // Use withTrashed() only if Product uses SoftDeletes
        // Query all products (parents and variations)
        $query = Product::query();
        if (in_array('Illuminate\Database\Eloquent\SoftDeletes', class_uses(Product::class))) {
            $query = $query->withTrashed();
        }

        // Apply other filters only if present
        if (!empty($this->filters['categories'])) {
            $query->whereHas('categories', function ($q) {
                $q->whereIn('ec_product_categories.id', $this->filters['categories']);
            });
        }
        ...
        if (!empty($this->filters['product_types'])) {
            $query->whereIn('product_type', $this->filters['product_types']);
        }
        if (!empty($this->filters['stock_status'])) {
            $query->whereIn('stock_status', $this->filters['stock_status']);
        }
    ...
    {
        // No longer used: all products are exported as flat rows
        return 0;
    }
