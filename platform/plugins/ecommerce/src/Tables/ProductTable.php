...
class ProductTable extends TableAbstract
{
        ...
        $this
            ...
            ->addColumns([
                ...
                Column::make('store_name')
                    ->title(trans('plugins/ecommerce::products.store_locator'))
                    ->alignStart(),
                Column::make('order')
            ...
            ->queryUsing(function (Builder $query) {
                return $query
                    ->select([
                        ...
                        'ec_store_locators.name as store_name',
                    ])
                    ->leftJoin('ec_store_locators', 'ec_products.store_id', '=', 'ec_store_locators.id')
    ...
    public function ajax(): JsonResponse
    {
        $data = $this->table
            ...
            ->editColumn('store_name', function (Product $item) {
                return BaseHelper::clean($item->store_name ?: '&mdash;');
            })
            ...
            ->addColumn('view', function (Product $item) {
                return '<a href="' . $item->url . '" target="_blank" class="btn btn-sm btn-primary"><i class="fa fa-eye"></i></a>';
            })
    ...
    public function getBulkChanges(): array
    {
        return [
            ...
            'store_id' => [
                ...
                'callback' => function (int|string|null $value = null): array {
                    $storeSelected = [];
                    if ($value && $store = StoreLocator::query()->find($value)) {
                        $storeSelected = [$store->getKey() => $store->name];
                    }

                    return [
                        'url' => route('store-locators.search'),
