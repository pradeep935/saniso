    ...
    public function boot(): void
    {
        ...
        $this->app['events']->listen(RenderingDashboardWidgets::class, function (): void {
            ...
            add_filter(DASHBOARD_FILTER_ADMIN_LIST, function ($widgets, $widgetSettings) {
                $items = DashboardCacheService::getOrderCount();
            ...
            add_filter(DASHBOARD_FILTER_ADMIN_LIST, function ($widgets, $widgetSettings) {
                $items = DashboardCacheService::getProductCount();
            ...
            add_filter(DASHBOARD_FILTER_ADMIN_LIST, function ($widgets, $widgetSettings) {
                $items = DashboardCacheService::getCustomerCount();
            ...
            add_filter(DASHBOARD_FILTER_ADMIN_LIST, function ($widgets, $widgetSettings) {
                $items = DashboardCacheService::getReviewCount();
            ...
            add_filter(DASHBOARD_FILTER_ADMIN_LIST, function ($widgets, $widgetSettings) {
                $items = DashboardCacheService::getQuoteCount();

                return (new DashboardWidgetInstance())
                    ->setType('stats')
                    ->setPermission('quote-requests.index')
                    ->setTitle(trans('plugins/ecommerce::ecommerce.quote_requests'))
                    ->setKey('widget_total_5')
                    ->setIcon('ti ti-file-text')
                    ->setColor('#28a745')
                    ->setStatsTotal($items)
                    ->setRoute(route('quote-requests.index'))
                    ->setColumn('col-12 col-md-6 col-lg-3')
                    ->init($widgets, $widgetSettings);
            }, 6, 2);

            add_filter(DASHBOARD_FILTER_ADMIN_LIST, function ($widgets, $widgetSettings) {
                $items = DashboardCacheService::getProjectCount();

                return (new DashboardWidgetInstance())
                    ->setType('stats')
                    ->setPermission('project-requests.index')
                    ->setTitle(trans('plugins/ecommerce::ecommerce.project_requests'))
                    ->setKey('widget_total_6')
                    ->setIcon('ti ti-building')
                    ->setColor('#007bff')
                    ->setStatsTotal($items)
                    ->setRoute(route('project-requests.index'))
                    ->setColumn('col-12 col-md-6 col-lg-3')
                    ->init($widgets, $widgetSettings);
            }, 7, 2);
        });
