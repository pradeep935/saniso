...
class ProductRepository extends RepositoriesAbstract implements ProductInterface
{
    ...
    public function filterProducts(array $filters, array $params = [])
    {
        ...
        // Channel availability logic for webshop (exclude all POS-only products)
        if (isset($filters['webshop_channel']) && $filters['webshop_channel'] === true) {
            $this->model = $this->model->where(function ($query) {
                $query->where('ec_products.available_in_webshop', true)
                    // If both are unchecked, available everywhere
                    ->orWhere(function ($q) {
                        $q->where(function ($subQ) {
                            $subQ->where(function ($q2) {
                                $q2->whereNull('ec_products.available_in_webshop')
                                    ->orWhere('ec_products.available_in_webshop', false);
                            })
                            ->where(function ($q3) {
                                $q3->whereNull('ec_products.available_in_pos')
                                    ->orWhere('ec_products.available_in_pos', false);
                            });
                        });
                    });
            })
            // Exclude all products that are POS-only (legacy or plugin field)
            ->where(function ($query) {
                $query->whereNull('ec_products.is_available_in_pos')
                    ->orWhere('ec_products.is_available_in_pos', false)
                    ->orWhere('ec_products.available_in_webshop', true);
            });
        }
                ...
                $this->model = $this->model
                    ->where(function (EloquentBuilder $query) use ($keyword, $searchProductsBy, $isPartial): void {
                        ...
                        if (in_array('barcode', $searchProductsBy)) {
                            ...
                            $query
                                ->{$function}(function (BaseQueryBuilder $subQuery) use ($keyword): void { // @phpstan-ignore-line
