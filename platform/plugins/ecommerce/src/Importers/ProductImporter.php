...
class ProductImporter extends Importer implements WithMapping
{
    ...
    protected bool $barcodeCacheLoaded = false;
    ...
    public function __construct()
    {
        ...
        // Disable query logging during import for better performance
        if (app()->environment('production')) {
            \Log::getLogger()->setHandlers([]);
        }
    }
    ...
    {
        // Do not validate uniqueness at the validator step to allow import to continue.
        // Duplicated barcodes will be detected during mapping and skipped with a failure message.
    ...
    public function map(mixed $row): array
    {
        ...
        // Mark barcode cache as loaded to skip the loadBarcodeCache call
        if (!$this->barcodeCacheLoaded) {
            $this->loadBarcodeCache();
        }
        ...
        // Optimized barcode validation using direct query (no memory cache)
        $barcode = Arr::get($row, 'barcode');
        if ($barcode && trim($barcode) !== '' && $barcode !== '0') {
            $barcode = trim($barcode);
            
            if (!$this->updateExisting) {
                // Use database query with index, not memory cache
                // This is fast because of the INDEX on barcode column
                ...
                if ($existingProduct) {
                    $this->onFailure(
                        $this->currentRow,
                        'Barcode',
                        [__('Barcode ":value" already exists for product ":product" (ID: :id). Enable "Update existing products" to modify it.', [
                            'value' => $barcode,
        ...
        // Remove keys that were not present in the uploaded file (they will be null).
        // This prevents missing columns from overwriting existing DB values during update.
    ...
    /**
     * Pre-load barcodes on-demand using database directly for validation
     * Don't load all into memory - query only when needed
     */
    protected function loadBarcodeCache(): void
    {
        // Mark as loaded but don't actually load anything into memory
        // We'll query the database directly when needed
        $this->barcodeCacheLoaded = true;
    }
    ...
    public function storeVariant(array $row, ?Product $product): ProductVariation|Model|null
    {
        ...
        if (! empty(Arr::get($version, 'description'))) {
            $version['description'] = BaseHelper::clean(Arr::get($version, 'description'));
        }

        if (! empty(Arr::get($version, 'content'))) {
            $version['content'] = BaseHelper::clean(Arr::get($version, 'content'));
        }
    ...
    {
        // PRESERVE EXISTING DATA: Only update images if provided in import
        $imagesInput = $request->input('images', []);
        
        if (!empty($imagesInput) && is_array($imagesInput) && count(array_filter($imagesInput)) > 0) {
            // Images provided in CSV - process them
            $images = $this->getImageURLs((array) $imagesInput);
        } else {
            // No images in CSV - preserve existing images
    ...
    protected function createTranslations(Product $product, array $row): void
    {
        ...
        foreach ($this->supportedLocales as $language) {
            ...
            $translationData = [
                'name' => Arr::get($row, "name_({$languageCode})", Arr::get($row, 'name')),
                'description' => Arr::get($row, "description_({$languageCode})", Arr::get($row, 'description')),
                'content' => Arr::get($row, "content_({$languageCode})", Arr::get($row, 'content')),
            ];
    ...
    public function mapLocalization(array $row): array
    {
        ...
        // Always generate SKU if missing
        if ($row['import_type'] == 'product' && empty($row['sku'])) {
    ...
    protected function setValue(array &$row, string $key, string $type = 'array', $default = null, $from = null): self
    {
        ...
        if ($value && $key == 'barcode' && trim($value) !== '' && $value !== '0') {
            // Clean the barcode value
            $value = trim($value);
            
            if (! $this->updateExisting) {
                // Check for duplicates within the current import batch
                if ($duplicateBarcode = $this->barcodes->firstWhere('value', $value)) {
                    \Log::info('Barcode duplicate within import batch detected', [
                        'barcode' => $value,
                        'current_row' => $this->currentRow,
                        'duplicate_row' => Arr::get($duplicateBarcode, 'row')
                    ]);
                    ...
                    $this->onFailure(
                        ...
                        [
                            __(
                                'Barcode ":value" has been duplicated within this import on row #:row',
                                ['value' => $value, 'row' => Arr::get($duplicateBarcode, 'row')]
                            ),
                        ],
                        [$value]
                    );
                } else {
                    // Add to tracking list for future duplicate detection
                    $this->barcodes->push(['row' => $this->currentRow, 'value' => $value]);
                    \Log::info('Barcode added to tracking list', [
                        'barcode' => $value,
                        'current_row' => $this->currentRow
                    ]);
                }
    ...
    protected function getImageURLs(array $images): array
    {
        ...
        // Check for download_images parameter from request, defaulting to true
        $downloadRemote = request()->boolean('download_images', true);
        
        \Log::info('Processing images for import', [
            'image_count' => count($images),
            'download_remote' => $downloadRemote,
            'images' => $images
        ]);
        ...
        foreach ($images as $key => $image) {
            ...
            // If image uses the media upload URL, store it as relative path (existing behavior)
            if (Str::startsWith($images[$key], RvMedia::getUploadURL())) {
                $images[$key] = str_replace(RvMedia::getUploadURL() . '/', '', $images[$key]);
                \Log::info('Image is local media URL, converted to relative path: ' . $images[$key]);
                continue;
            }

            // If the image is a remote URL (http/https) and download_images is enabled, download it.
            if ($downloadRemote && Str::startsWith($images[$key], ['http://', 'https://'])) {
                \Log::info('Downloading remote image: ' . $images[$key]);
                $images[$key] = $this->uploadImageFromURL($images[$key]);
            } else {
                \Log::info('Image URL kept as provided (no download): ' . $images[$key]);
            }

            // Otherwise, keep the URL or relative path as provided (no download).
        }
        
        \Log::info('Final processed images: ', $images);
    ...
    protected function uploadImageFromURL(?string $url): ?string
    {
        if (empty($url) || !filter_var($url, FILTER_VALIDATE_URL)) {
            \Log::warning('Invalid URL provided for image upload: ' . $url);
            return null;
        }

        try {
            \Log::info('Attempting to upload image from URL: ' . $url);
            
            // Use Laravel HTTP client with proper headers to avoid 403 Forbidden
            ...
            if ($response->failed() || !$response->body()) {
                \Log::warning('Failed to download image: ' . $url . ' Status: ' . $response->status() . ' Reason: ' . $response->reason());
            ...
            $filename = ($info['filename'] ?? 'image_' . time()) . '.' . $extension;
            
            // Create temporary file
            $tempPath = sys_get_temp_dir() . '/' . uniqid('import_image_') . '_' . $filename;
            file_put_contents($tempPath, $response->body());
            
            \Log::info('Downloaded image to temp file: ' . $tempPath . ' Size: ' . filesize($tempPath));

            // Upload the temporary file using RvMedia
            $result = RvMedia::uploadFromPath($tempPath, 0, 'products');

            // Clean up temporary file
            @unlink($tempPath);
            
            \Log::info('RvMedia upload result structure:', [
                'result' => $result,
                'data_url' => $result['data']->url ?? 'N/A',
                'data_type' => gettype($result['data'] ?? null)
            ]);

            if (!$result['error']) {
                \Log::info('Image upload successful: ' . $result['data']->url);
                
                // The result should contain the proper relative path for database storage
                $imagePath = $result['data']->url;
                
                // Ensure we always save relative paths, not full URLs
                // Remove any S3 domain/URL prefixes to get just the relative path
                if (str_contains($imagePath, 'saniso-media.s3.eu-north-1.amazonaws.com')) {
                    // Extract just the relative path from S3 URL
                    $imagePath = str_replace('https://saniso-media.s3.eu-north-1.amazonaws.com/', '', $imagePath);
                } elseif (str_contains($imagePath, 'amazonaws.com')) {
                    // Handle other S3 URLs
                    $pathParts = explode('/', $imagePath);
                    $filename = end($pathParts);
                    $imagePath = 'products/' . $filename;
                } elseif (Str::startsWith($imagePath, 'http')) {
                    // Handle any other full URLs
                    $pathParts = explode('/', $imagePath);
                    $filename = end($pathParts);
                    $imagePath = 'products/' . $filename;
                } elseif (Str::startsWith($imagePath, RvMedia::getUploadURL())) {
                    // Remove the upload URL prefix if present
                    $imagePath = str_replace(RvMedia::getUploadURL() . '/', '', $imagePath);
                }
                
                // Ensure the path starts with 'products/' for consistency
                if (!Str::startsWith($imagePath, 'products/')) {
                    $filename = basename($imagePath);
                    $imagePath = 'products/' . $filename;
                }
                
                \Log::info('Final processed image path for database: ' . $imagePath);
                return $imagePath;
            } else {
                \Log::error('RvMedia upload failed: ' . $result['message'] . ' for URL: ' . $url);
                return $url; // Return original URL as fallback
            }
        } catch (\Exception $e) {
            \Log::error('Exception during image upload: ' . $e->getMessage() . ' for URL: ' . $url);
            return $url; // Return original URL as fallback
        }
    }
