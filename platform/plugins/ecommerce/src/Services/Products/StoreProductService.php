...
class StoreProductService
{
    public function execute(Request $request, Product $product, bool $forceUpdateAll = false): Product
    {
        ...
        // Determine whether we need to persist attribute changes to avoid redundant DB writes.
        // We only skip saving when updating an existing product, not when creating new one,
        // and only when caller didn't force a full update.
        $dirty = $product->getDirty();

        $shouldSave = true;

        if ($exists && ! $forceUpdateAll) {
            $hasProductFilesInput = $request->hasFile('product_files_input');
            $hasExternalFiles = ! empty((array) $request->input('product_files_external', []));
            $hasOptions = $request->has('has_product_options');
            $hasSpecs = $request->has('specification_attributes');

            if (empty($dirty) && ! $hasProductFilesInput && ! $hasExternalFiles && ! $hasOptions && ! $hasSpecs) {
                $shouldSave = false;
            }
        }
        ...
        if ($shouldSave) {
            ...
            if (config('app.debug')) {
                \Log::debug('Skipping product save for id ' . $product->id . ' - no attribute changes detected.');
            }
        }
        ...
        if ($isDefaultLanguage) {
            \Log::info('Processing specification attributes', [
                'product_id' => $product->id,
                'attributes_data' => $request->input('specification_attributes', []),
                'has_files' => $request->hasFile('specification_attributes'),
                'all_files' => $request->allFiles(),
                'request_method' => $request->method(),
                'content_type' => $request->header('Content-Type')
            ]);
            
            // Batch-load attributes to avoid N+1 queries and reduce logging I/O.
            $incoming = $request->collect('specification_attributes');
            $attributeIds = $incoming->keys()->filter()->values()->all();

            $attributesMap = SpecificationAttribute::query()
                ->whereIn('id', $attributeIds)
                ->get()
                ->keyBy('id');

            $specificationAttributes = $incoming->mapWithKeys(function ($item, $key) use ($attributesMap) {
            ...
            // Reduce verbosity: log at debug level and only when debug is enabled.
            if (config('app.debug')) {
                \Log::debug('Syncing specification attributes', [
                    'product_id' => $product->id,
                    'attributes_to_sync' => $specificationAttributes->toArray(),
                ]);
            }

            $syncResult = $product->specificationAttributes()->sync($specificationAttributes);

            if (config('app.debug')) {
                \Log::debug('Sync completed for specification attributes', [
                    'product_id' => $product->id,
