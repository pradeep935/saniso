...
<div class="row price-group">
    ...
    <div class="col-md-6">
        <label class="form-label fw-semibold mb-2">Product Channel Availability</label>
        <div class="form-check mb-2">
            <input type="checkbox" name="available_in_pos" class="form-check-input" id="available_in_pos" value="1" @if(old('available_in_pos', $product ? $product->available_in_pos : false)) checked @endif>
            <label class="form-check-label" for="available_in_pos">Available in POS</label>
        </div>
        <div class="form-check mb-2">
            <input type="checkbox" name="available_in_webshop" class="form-check-input" id="available_in_webshop" value="1" @if(old('available_in_webshop', $product ? $product->available_in_webshop : false)) checked @endif>
            <label class="form-check-label" for="available_in_webshop">Available in Webshop</label>
        </div>
        <div class="small text-muted mt-2">
            <ul class="mb-0">
                <li>If both are unchecked, product is available in both POS and Webshop.</li>
                <li>If only one is checked, product is only available in that channel.</li>
                <li>If both are checked, product is available in both.</li>
            </ul>
        </div>
    </div>
</div>
...
<x-core::form.fieldset class="storehouse-info" @style(['display: none' => old('with_storehouse_management', $product ? $product->with_storehouse_management : $originalProduct->with_storehouse_management ?? 0) == 0])>
    ...
    <x-core::form.on-off.checkbox
        ...
        :checked="old('allow_checkout_when_out_of_stock', $product ? $product->allow_checkout_when_out_of_stock : $originalProduct->allow_checkout_when_out_of_stock ?? 1) == 1"
    />
