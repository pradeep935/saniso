...
@php
    ...
    $isLargeExport = $totalItems > 20000;
    $isMediumExport = $totalItems > 1000 && $totalItems <= 20000;
@endphp
    ...
    <div class="card mb-4">
        <div class="card-header">
            <h5 class="card-title mb-0">
                <i class="fa fa-filter me-2"></i>{{ trans('plugins/ecommerce::products.export.filter_products') }}
            </h5>
            <p class="text-muted small mb-0">{{ trans('plugins/ecommerce::products.export.filter_products_description') }}</p>
        </div>
        <div class="card-body">
            <div class="row">
                <!-- Categories Filter -->
                <div class="col-md-6 mb-3">
                    <x-core::form-group>
                        <x-core::form.label>{{ trans('plugins/ecommerce::products.form.categories') }}</x-core::form.label>
                        <select name="filter_categories[]" id="filter_categories" class="form-control select2" multiple>
                            @foreach(\Botble\Ecommerce\Models\ProductCategory::orderBy('name')->get() as $category)
                                <option value="{{ $category->id }}">{{ $category->name }}</option>
                            @endforeach
                        </select>
                        <x-core::form.helper-text>{{ trans('plugins/ecommerce::products.export.categories_helper') }}</x-core::form.helper-text>
                    </x-core::form-group>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <x-core::form-group>
                        <x-core::form.label>{{ trans('plugins/ecommerce::products.form.brand') }}</x-core::form.label>
                        <select name="filter_brands[]" id="filter_brands" class="form-control select2" multiple>
                            @foreach(\Botble\Ecommerce\Models\Brand::orderBy('name')->get() as $brand)
                                <option value="{{ $brand->id }}">{{ $brand->name }}</option>
                            @endforeach
                        </select>
                        <x-core::form.helper-text>{{ trans('plugins/ecommerce::products.export.brands_helper') }}</x-core::form.helper-text>
                    </x-core::form-group>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <x-core::form-group>
                        <x-core::form.label>{{ trans('core/base::tables.status') }}</x-core::form.label>
                        <select name="filter_status[]" id="filter_status" class="form-control select2" multiple>
                            <option value="published">{{ trans('core/base::enums.statuses.published') }}</option>
                            <option value="draft">{{ trans('core/base::enums.statuses.draft') }}</option>
                            <option value="pending">{{ trans('core/base::enums.statuses.pending') }}</option>
                        </select>
                        <x-core::form.helper-text>{{ trans('plugins/ecommerce::products.export.status_helper') }}</x-core::form.helper-text>
                    </x-core::form-group>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <x-core::form-group>
                        <x-core::form.label>{{ trans('plugins/ecommerce::products.form.product_type.title') }}</x-core::form.label>
                        <select name="filter_product_types[]" id="filter_product_types" class="form-control select2" multiple>
                            <option value="physical">{{ trans('plugins/ecommerce::products.form.product_type.physical') }}</option>
                            <option value="digital">{{ trans('plugins/ecommerce::products.form.product_type.digital') }}</option>
                        </select>
                        <x-core::form.helper-text>{{ trans('plugins/ecommerce::products.export.product_type_helper') }}</x-core::form.helper-text>
                    </x-core::form-group>
                </div>
                @endif
                ...
                <div class="col-md-6 mb-3">
                    <x-core::form-group>
                        <x-core::form.label>{{ trans('plugins/ecommerce::products.form.stock_status') }}</x-core::form.label>
                        <select name="filter_stock_status[]" id="filter_stock_status" class="form-control select2" multiple>
                            <option value="in_stock">{{ trans('plugins/ecommerce::products.form.stock_statuses.in_stock') }}</option>
                            <option value="out_of_stock">{{ trans('plugins/ecommerce::products.form.stock_statuses.out_of_stock') }}</option>
                            <option value="on_backorder">{{ trans('plugins/ecommerce::products.form.stock_statuses.on_backorder') }}</option>
                        </select>
                        <x-core::form.helper-text>{{ trans('plugins/ecommerce::products.export.stock_status_helper') }}</x-core::form.helper-text>
                    </x-core::form-group>
                </div>
                ...
                <div class="col-md-6 mb-3">
                    <x-core::form-group>
                        <x-core::form.label>{{ trans('plugins/ecommerce::products.form.is_featured') }}</x-core::form.label>
                        <select name="filter_is_featured" id="filter_is_featured" class="form-control">
                            <option value="">{{ trans('plugins/ecommerce::products.export.all_products') }}</option>
                            <option value="1">{{ trans('plugins/ecommerce::products.export.featured_only') }}</option>
                            <option value="0">{{ trans('plugins/ecommerce::products.export.non_featured_only') }}</option>
                        </select>
                        <x-core::form.helper-text>{{ trans('plugins/ecommerce::products.export.featured_helper') }}</x-core::form.helper-text>
                    </x-core::form-group>
                </div>
            </div>
            ...
            <div class="row">
                <div class="col-12">
                    <div class="alert alert-info">
                        ...
                        {{ trans('plugins/ecommerce::products.export.filter_info') }}
                    </div>
                    
                    <!-- Dynamic Export Count -->
                    <div class="alert alert-success" id="export-count-display" style="display: none;">
                        <div class="d-flex align-items-center justify-content-between">
                            <div>
                                <i class="fa fa-download me-2"></i>
                                <strong id="export-count-text">{{ trans('plugins/ecommerce::products.export.products_to_export', ['count' => 0]) }}</strong>
                            </div>
                            <button type="button" class="btn btn-sm btn-outline-success" id="refresh-count">
                                <i class="fa fa-refresh"></i> {{ trans('plugins/ecommerce::products.export.refresh_count') }}
                            </button>
                        </div>
                        <div class="mt-2">
                            <small id="export-breakdown" class="text-muted"></small>
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="col-md-6">
            <div class="mb-3">
                ...
                <div class="form-text text-muted small">
                    ...
                    {{ trans('plugins/ecommerce::products.export.include_variations_helper') }}
                </div>
            </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if (typeof $.fn.select2 !== 'undefined') {
                $('#filter_categories').select2({
                    placeholder: '{{ trans('plugins/ecommerce::products.export.select_categories') }}',
                    allowClear: true,
                    width: '100%'
                }).on('change', updateExportCount);

                $('#filter_brands').select2({
                    placeholder: '{{ trans('plugins/ecommerce::products.export.select_brands') }}',
                    allowClear: true,
                    width: '100%'
                }).on('change', updateExportCount);

                $('#filter_status').select2({
                    placeholder: '{{ trans('plugins/ecommerce::products.export.select_status') }}',
                    allowClear: true,
                    width: '100%'
                }).on('change', updateExportCount);

                $('#filter_product_types').select2({
                    placeholder: '{{ trans('plugins/ecommerce::products.export.select_product_types') }}',
                    allowClear: true,
                    width: '100%'
                }).on('change', updateExportCount);

                $('#filter_stock_status').select2({
                    placeholder: '{{ trans('plugins/ecommerce::products.export.select_stock_status') }}',
                    allowClear: true,
                    width: '100%'
                }).on('change', updateExportCount);
            }

            // Also listen to featured filter changes
            $('#filter_is_featured').on('change', updateExportCount);

            // Refresh count button
            $('#refresh-count').on('click', updateExportCount);

            // Function to update export count
            function updateExportCount() {
                const $countDisplay = $('#export-count-display');
                const $countText = $('#export-count-text');
                const $breakdown = $('#export-breakdown');
                const $refreshBtn = $('#refresh-count');

                // Show loading state
                $refreshBtn.html('<i class="fa fa-spinner fa-spin"></i> {{ trans('plugins/ecommerce::products.export.updating') }}');
                $refreshBtn.prop('disabled', true);

                // Collect filter data
                const filterData = {
                    filter_categories: $('#filter_categories').val() || [],
                    filter_brands: $('#filter_brands').val() || [],
                    filter_status: $('#filter_status').val() || [],
                    filter_product_types: $('#filter_product_types').val() || [],
                    filter_stock_status: $('#filter_stock_status').val() || [],
                    filter_is_featured: $('#filter_is_featured').val() || '',
                    include_variations: $('input[name="include_variations"]').is(':checked') ? 1 : 0
                };

                // Make AJAX request to get count
                $.ajax({
                    url: '{{ route('tools.data-synchronize.export.products.count') }}',
                    method: 'POST',
                    data: filterData,
                    headers: {
                    ...
                    success: function(response) {
                        if (response.success) {
                            ...
                            $countText.text('{{ trans('plugins/ecommerce::products.export.products_to_export_prefix') }} ' + count.toLocaleString() + ' {{ trans('plugins/ecommerce::products.export.products_to_export_suffix') }}');
                            
                            let breakdownText = '';
                            if (breakdown.products > 0) {
                                breakdownText += breakdown.products.toLocaleString() + ' {{ trans('plugins/ecommerce::products.export.products') }}';
                            }
                            if (breakdown.variations > 0) {
                                if (breakdownText) breakdownText += ' + ';
                                breakdownText += breakdown.variations.toLocaleString() + ' {{ trans('plugins/ecommerce::products.export.variations') }}';
                            }
                            
                            $breakdown.text(breakdownText);
                            $countDisplay.show();
                        } else {
                            $countText.text('{{ trans('plugins/ecommerce::products.export.count_error') }}');
                            $breakdown.text('');
                            $countDisplay.show();
                        }
                    },
                    ...
                    complete: function() {
                        $refreshBtn.html('<i class="fa fa-refresh"></i> {{ trans('plugins/ecommerce::products.export.refresh_count') }}');
            ...
            setTimeout(updateExportCount, 500);
