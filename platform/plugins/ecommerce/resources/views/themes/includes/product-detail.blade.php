    ...
    <div class="row">
        ...
        <div class="col-lg-6 col-md-6 col-sm-12 mb-30">
            <div class="bb-product-page-content">
                @php
                    // prepare a robust next-product URL for top-right button (category -> store -> global)
                    $nextProductUrl = null;
                    try {
                        $contextCategory = $product->categories->sortByDesc('id')->first();

                        if ($contextCategory) {
                            $found = \Botble\Ecommerce\Models\Product::query()
                                ->wherePublished()
                                ->whereHas('categories', function ($q) use ($contextCategory) {
                                    $q->where('ec_product_categories.id', $contextCategory->id);
                                })
                                ->where('id', '>', $product->id)
                                ->orderBy('id')
                                ->first();
                            ...
                            if ($found) {
                                $nextProductUrl = $found->url;
                            }
                        }

                        if (! $nextProductUrl && ! empty($product->store_id)) {
                            $found = \Botble\Ecommerce\Models\Product::query()
                                ->wherePublished()
                        ...
                        // swallow any unexpected errors so the product page still renders
                        $nextProductUrl = null;
                    }
                @endphp
                ...
                <div class="d-flex justify-content-between align-items-start">
                    ...
                    {{-- debug: next-product-url: {{ $nextProductUrl ?? 'null' }} --}}
                    @if (! empty($nextProductUrl))
                        <div class="product-next-btn ms-3">
                            <a href="{{ $nextProductUrl }}" class="btn btn-outline-secondary">{{ __('Next product') }}</a>
                        </div>
                    @endif
                </div>
