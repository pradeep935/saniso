<div class="row cart-item">
    ...
    <div class="col">
        ...
        <p class="mb-0">
            ...
            @if (Arr::get($cartItem->options, 'is_quote_item'))
                <span class="badge bg-warning ms-2">{{ __('Quote Item') }}</span>
            @endif
        ...
        @if (get_ecommerce_setting('checkout_product_quantity_editable', true) && !Arr::get($cartItem->options, 'is_quote_item'))
            <div
        ...
        @elseif (Arr::get($cartItem->options, 'is_quote_item'))
            <div class="ec-checkout-quote-info">
                <p class="mb-1">
                    <small class="text-muted">
                        {{ __('Quote Quantity') }}: <strong>{{ $cartItem->qty }}</strong>
                        <span class="text-warning">({{ __('Fixed - Cannot be modified') }})</span>
                    </small>
                </p>
                @if (Arr::get($cartItem->options, 'quote_id'))
                    <p class="mb-1">
                        <small class="text-muted">
                            {{ __('Quote ID') }}: <strong>#{{ Arr::get($cartItem->options, 'quote_id') }}</strong>
                        </small>
                    </p>
                @endif
            </div>
        ...
        @if (!empty($cartItem->options['options']) && isset($cartItem->options['options']['optionCartValue']))
    ...
    <div class="col-auto text-end">
        @if (Arr::get($cartItem->options, 'is_quote_item'))
            <div class="quote-pricing">
                @if (Arr::get($cartItem->options, 'original_price') && Arr::get($cartItem->options, 'original_price') != $cartItem->price)
                    <p class="mb-1">
                        <small class="text-muted text-decoration-line-through">
                            {{ __('Original') }}: {{ format_price(Arr::get($cartItem->options, 'original_price')) }}
                        </small>
                    </p>
                @endif
        ...
        @else
            <p class="mb-0">
                <small class="text-muted">
                    ...
                    @if (Arr::get($cartItem->options, 'is_quote_item'))
                        <span class="text-warning">({{ __('Quote Final Total') }})</span>
                    @endif
                </small>
            </p>
        @endif
