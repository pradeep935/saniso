                        ...
                        <div class="row">
                            <div class="col-md-6 offset-md-6">
                                ...
                                <div class="btn-list justify-content-end my-3">
                                    {{-- Picking List Buttons --}}
                                    @if ($order->status != \Botble\Ecommerce\Enums\OrderStatusEnum::CANCELED)
                                        <x-core::button
                                            tag="a"
                                            href="{{ route('orders.generate-picking-list', $order->id) }}?type=print"
                                            target="_blank"
                                            icon="ti ti-list-check"
                                            color="warning"
                                        >
                                            {{ trans('plugins/ecommerce::order.print_picking_list') }}
                                        </x-core::button>
                                        <x-core::button
                                            tag="a"
                                            :href="route('orders.generate-picking-list', $order->id)"
                                            target="_blank"
                                            icon="ti ti-download"
                                            color="warning"
                                            variant="outline"
                                        >
                                            {{ trans('plugins/ecommerce::order.download_picking_list') }}
                                        </x-core::button>
                                    @endif
